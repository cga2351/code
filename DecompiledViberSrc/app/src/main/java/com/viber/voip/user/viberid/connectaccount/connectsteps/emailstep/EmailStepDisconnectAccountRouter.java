package com.viber.voip.user.viberid.connectaccount.connectsteps.emailstep;

import com.viber.voip.user.editinfo.diconnectaccount.DisconnectAccountRouter;

class EmailStepDisconnectAccountRouter
  implements DisconnectAccountRouter
{
  private final RetypePasswordDialogView mView;

  EmailStepDisconnectAccountRouter(RetypePasswordDialogView paramRetypePasswordDialogView)
  {
    this.mView = paramRetypePasswordDialogView;
  }

  public void navigateOnDisconnect()
  {
    this.mView.closeRetypePasswordDialog();
  }
}

/* Location:           E:\Study\Tools\apktool2_2\dex2jar-0.0.9.15\classes_viber_4_dex2jar.jar
 * Qualified Name:     com.viber.voip.user.viberid.connectaccount.connectsteps.emailstep.EmailStepDisconnectAccountRouter
 * JD-Core Version:    0.6.2
 */