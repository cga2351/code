  ...
  implements Producer<EncodedImage>
{
  ...
  private static TriState shouldTranscode(EncodedImage paramEncodedImage)
  {
    ...
    ImageFormat localImageFormat = ImageFormatChecker.getImageFormat_WrapIOException(paramEncodedImage.getInputStream());
    if (DefaultImageFormats.isStaticWebpFormat(localImageFormat))
    {
      WebpTranscoder localWebpTranscoder = WebpTranscoderFactory.getWebpTranscoder();
      if (localWebpTranscoder == null)
        return TriState.NO;
      if (!localWebpTranscoder.isWebpNativelySupported(localImageFormat));
  ...
  private void transcodeLastResult(EncodedImage paramEncodedImage, Consumer<EncodedImage> paramConsumer, ProducerContext paramProducerContext)
  {
    ...
    StatefulProducerRunnable local1 = new StatefulProducerRunnable(paramConsumer, paramProducerContext.getListener(), "WebpTranscodeProducer", paramProducerContext.getId())
    {
        ...
        throws Exception
      {
        ...
        try
        {
          WebpTranscodeProducer.doTranscode(localEncodedImage, localPooledByteBufferOutputStream);
          ...
          try
          {
            ...
            localEncodedImage.copyMetaDataFrom(localEncodedImage);
