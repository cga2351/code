    ...
    implements Runnable
  {
    ...
    public void run()
    {
      ...
      try
      {
        ...
        while (true)
        {
          j = ConstrainedExecutorService.this.mPendingWorkers.decrementAndGet();
      ...
      {
        throw localObject;
        FLog.v(ConstrainedExecutorService.TAG, "%s: worker finished; %d workers left", ConstrainedExecutorService.this.mName, Integer.valueOf(j));
        return;
        label136: FLog.v(ConstrainedExecutorService.TAG, "%s: worker finished; %d workers left", ConstrainedExecutorService.this.mName, Integer.valueOf(i));
      }
    }
  }
}
