  ...
  implements CloneableDrawable
{
  ...
  private void drawHorizontalBar(Canvas paramCanvas, int paramInt1, int paramInt2)
  {
    ...
    int i = paramInt1 * (localRect.width() - 2 * this.mPadding) / 10000;
    int j = localRect.left + this.mPadding;
    int k = localRect.bottom - this.mPadding - this.mBarWidth;
    this.mRect.set(j, k, i + j, k + this.mBarWidth);
  ...
  {
    Rect localRect = getBounds();
    int i = paramInt1 * (localRect.height() - 2 * this.mPadding) / 10000;
    int j = localRect.left + this.mPadding;
    int k = localRect.top + this.mPadding;
    this.mRect.set(j, k, j + this.mBarWidth, k + i);
    drawBar(paramCanvas, paramInt2);
  }
