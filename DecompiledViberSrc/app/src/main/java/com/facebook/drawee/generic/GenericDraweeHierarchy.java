  ...
  implements SettableDraweeHierarchy
{
  ...
  GenericDraweeHierarchy(GenericDraweeHierarchyBuilder paramGenericDraweeHierarchyBuilder)
  {
    ...
    label268: for (int k = 1; ; k = 0)
    {
      int m = j + k;
      arrayOfDrawable = new Drawable[m + 6];
      ...
      arrayOfDrawable[2] = buildActualImageBranch(this.mActualImageWrapper, paramGenericDraweeHierarchyBuilder.getActualImageScaleType(), paramGenericDraweeHierarchyBuilder.getActualImageFocusPoint(), paramGenericDraweeHierarchyBuilder.getActualImageColorFilter());
      ...
      while (localIterator.hasNext())
      {
        ...
        int n = i + 1;
        arrayOfDrawable[(i + 6)] = buildBranch(localDrawable, null);
        i = n;
      }
      j = 1;
      break;
    ...
    label274: i = 1;
    if (paramGenericDraweeHierarchyBuilder.getPressedStateOverlay() != null)
      arrayOfDrawable[(i + 6)] = buildBranch(paramGenericDraweeHierarchyBuilder.getPressedStateOverlay(), null);
    label299: this.mFadeDrawable = new FadeDrawable(arrayOfDrawable);
  ...
  {
    Object localObject = this.mFadeDrawable.getDrawableParentForIndex(paramInt);
    if ((((DrawableParent)localObject).getDrawable() instanceof MatrixDrawable))
      localObject = (MatrixDrawable)((DrawableParent)localObject).getDrawable();
    if ((((DrawableParent)localObject).getDrawable() instanceof ScaleTypeDrawable))
      localObject = (ScaleTypeDrawable)((DrawableParent)localObject).getDrawable();
    return localObject;
  }
  ...
  private void setChildDrawableAtIndex(int paramInt, @Nullable Drawable paramDrawable)
  {
    ...
    Drawable localDrawable = WrappingUtils.maybeApplyLeafRounding(paramDrawable, this.mRoundingParams, this.mResources);
    getParentDrawableAtIndex(paramInt).setDrawable(localDrawable);
  }
  ...
  public void setImage(Drawable paramDrawable, float paramFloat, boolean paramBoolean)
  {
    ...
    this.mActualImageWrapper.setDrawable(localDrawable);
    this.mFadeDrawable.beginBatchMode();
    fadeOutBranches();
    fadeInLayer(2);
