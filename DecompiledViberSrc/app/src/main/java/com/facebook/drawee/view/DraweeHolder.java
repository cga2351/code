  ...
  implements VisibilityCallback
{
  ...
  {
    DraweeHolder localDraweeHolder = new DraweeHolder(paramDH);
    localDraweeHolder.registerWithContext(paramContext);
    return localDraweeHolder;
  }
  ...
  public void onDraw()
  {
    ...
    arrayOfObject[1] = Integer.valueOf(System.identityHashCode(this.mController));
    arrayOfObject[2] = toString();
    FLog.w(DraweeEventTracker.class, "%x: Draw requested for a non-attached controller %x. %s", arrayOfObject);
    this.mIsHolderAttached = true;
