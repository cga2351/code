  ...
  implements LifecycleEventListener, HeadlessJsTaskEventListener
{
  ...
  public void createTimer(int paramInt1, int paramInt2, double paramDouble, boolean paramBoolean)
  {
    ...
    long l3 = Math.max(0L, l2 - l1 + paramInt2);
    if ((paramInt2 == 0) && (!paramBoolean))
    {
      WritableArray localWritableArray = Arguments.createArray();
      localWritableArray.pushInt(paramInt1);
      ((JSTimers)getReactApplicationContext().getJSModule(JSTimers.class)).callTimers(localWritableArray);
      return;
    }
    Timer localTimer = new Timer(paramInt1, l3 + SystemClock.nanoTime() / 1000000L, paramInt2, paramBoolean, null);
    ...
    implements Runnable
  {
    ...
    public void run()
    {
      ...
      do
      {
        ...
        l3 = SystemClock.currentTimeMillis() - l2;
      }
      while (16.666666F - (float)l2 < 1.0F);
      synchronized (Timing.this.mIdleCallbackGuard)
      {
        boolean bool = Timing.this.mSendIdleEvents;
        if (bool)
          ((JSTimers)Timing.this.getReactApplicationContext().getJSModule(JSTimers.class)).callIdleCallbacks(l3);
  ...
  private class TimerFrameCallback extends ChoreographerCompat.FrameCallback
  {
    ...
    public void doFrame(long paramLong)
    {
      ...
      while (true)
      {
        ...
        {
          if ((Timing.this.mTimers.isEmpty()) || (((Timing.Timer)Timing.access$300(Timing.this).peek()).mTargetTime >= l))
            break;
          localTimer = (Timing.Timer)Timing.this.mTimers.poll();
          if (this.mTimersToCall == null)
            this.mTimersToCall = Arguments.createArray();
          this.mTimersToCall.pushInt(localTimer.mCallbackID);
          if (localTimer.mRepeat)
          {
            Timing.Timer.access$402(localTimer, l + localTimer.mInterval);
            Timing.this.mTimers.add(localTimer);
          }
        }
        Timing.this.mTimerIdsToTimers.remove(localTimer.mCallbackID);
      }
      if (this.mTimersToCall != null)
      {
