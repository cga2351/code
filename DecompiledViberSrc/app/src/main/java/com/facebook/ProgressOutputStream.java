    ...
    if (this.batchProgress > this.lastReportedProgress)
    {
      ...
      while (localIterator.hasNext())
      {
        ...
        if ((localCallback instanceof RequestBatch.OnProgressCallback))
        {
          ...
          final RequestBatch.OnProgressCallback localOnProgressCallback = (RequestBatch.OnProgressCallback)localCallback;
          if (localHandler == null)
            ...
            localHandler.post(new Runnable()
            {
              public void run()
              {
                localOnProgressCallback.onBatchProgress(ProgressOutputStream.this.requests, ProgressOutputStream.this.batchProgress, ProgressOutputStream.this.maxProgress);
              }
            });
        }
      }
