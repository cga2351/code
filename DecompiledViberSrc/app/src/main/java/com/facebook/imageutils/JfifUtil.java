...
public class JfifUtil
{
  ...
  public static int getOrientation(InputStream paramInputStream)
  {
    try
    {
      ...
      if (i == 0)
        return 0;
      int j = TiffUtil.readOrientationFromTIFF(paramInputStream, i);
      return j;
    }
    ...
    throws IOException
  {
    ...
    {
      int i = -2 + StreamProcessor.readPackedInt(paramInputStream, 2, false);
      if (i > 6)
      {
        int j = StreamProcessor.readPackedInt(paramInputStream, 4, false);
        int k = i - 4;
        int m = StreamProcessor.readPackedInt(paramInputStream, 2, false);
        int n = k - 2;
        if ((j == 1165519206) && (m == 0))
          return n;
      }
    ...
    throws IOException
  {
    ...
    while (StreamProcessor.readPackedInt(paramInputStream, 1, false) == 255)
    {
      ...
      if ((i != 216) && (i != 1))
      {
        if ((i == 217) || (i == 218))
          return false;
        paramInputStream.skip(-2 + StreamProcessor.readPackedInt(paramInputStream, 2, false));
      }
    }
    return false;
  }
